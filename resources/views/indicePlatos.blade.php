...
<body>
    ...
    <table>
        ...
        <tr>
            <td>{{$plato->nombre}}</td>
            <td>{{$plato->precio}}</td>
            <td><a href="{{ route('calcularPrecioPedido', $plato->precio) }}">Añadir</a></td>
        </tr>
        @endforeach
    </table>

    @if(session()->has('precioTotal'))
    <h2>Total pedido: ${{ session('precioTotal') }}</h2>
    @endif
    ...
    <a id="enlaceBoton" href="{{route('rutaFormularioCliente')}}"> <button>Finalizar pedido</button></a>
